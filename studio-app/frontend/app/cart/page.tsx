...
export default function CartPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      ...
      setSignedIn(ok);
      if (!ok) router.replace("/sign-in?next=/cart");
      else void loadAll();
    });
    return () => {
      cancelled = true;
  ...
  async function setQuantity(itemId: string, q: number) {
    try {
      const c = await apiPatch<HydratedCart>(`/cart/items/${itemId}`, {
        quantity: q,
      });
      setCart(c);
    } catch (e) {
  ...
  async function checkout() {
    ...
    try {
      ...
      // The literal {ORDER_ID} token is substituted server-side, where
      // the real order id is created. Stripe never sees the placeholder.
      const res = await apiPost<{ order_id: string; checkout_url: string }>(
        "/checkout",
        {
  ...
  if (!cart) {
    return error ? <Errored msg={error} /> : <Loading />;
  }
